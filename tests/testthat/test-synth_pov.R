...
test_that("get correct results", {
  ...
  ca_ag1 <- synthACS:::synth_data_pov(synthACS:::synth_data_nativ(
    synthACS:::synth_data_emp(synthACS:::synth_data_edu(synthACS:::synth_data_mar(
  ...
  expect_equal(levels(ca_ag1[[1]]$edu_attain), ca_ag2[[2]])
  ...
  expect_equal(tapply(ca_ag2[[1]]$p, ca_ag2[[1]]$emp_status, sum),
               tapply(ca[[1]]$p, ca[[1]]$emp_status, sum))
