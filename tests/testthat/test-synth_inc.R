...
test_that("get correct results", {
  ...
  ca_ag3 <- synthACS:::synth_data_inc(
    synthACS:::synth_data_geomob(synthACS:::synth_data_pov(synthACS:::synth_data_nativ(
    synthACS:::synth_data_emp(synthACS:::synth_data_edu(synthACS:::synth_data_mar(
  ...
  expect_equal(names(ca_ag1[[1]]), 
               c("age", "gender", "marital_status", "edu_attain", "emp_status", "nativity", 
                 "pov_status", "geog_mobility", "ind_income", "p"))
  ...
  expect_equal(tapply(ca_ag2[[1]]$p, ca_ag2[[1]]$pov_status, sum),
               tapply(ca[[1]]$p, ca[[1]]$pov_status, sum))
  # expect_equal(tapply(ca_ag2[[1]]$p, ca_ag2[[1]]$nativity, sum),
  #              tapply(ca[[1]]$p, ca[[1]]$nativity, sum)) # doesn't work due to renormalizing.
