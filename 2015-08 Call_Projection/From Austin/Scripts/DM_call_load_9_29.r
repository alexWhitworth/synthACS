function(called_data, current_date) {
    ...
    campaign_response <- sqlQuery(ch, "SELECT * FROM [c2g_campaign_tracker].[dbo].[c2g_campaign_response]")
    ...
    first_future_date <- max(as.numeric(as.Date(campaign_response_1$response_date)), na.rm="TRUE")
    
    #Now, limit to years and months we want only 
    campaign_response_2 <- 
      campaign_response_1[campaign_response_1$year_response_date > 2014 | 
                            campaign_response_1$year_response_date == 2014 & campaign_response_1$month_response_date >= 3, ]
    
    ###TRY ONLY > first future date so that we don't double count calls on future date
    campaign_future <- campaign_response_1[is.na(campaign_response_1$year_response_date) == "TRUE" & 
                           as.numeric(as.Date(campaign_response_1$date)) > first_future_date & 
    ...
    campaigns_outstanding <- 
      campaign_response_2[(current_date_numeric - as.numeric(as.Date(campaign_response_2$date))) <= 
                            (90 + (current_date_numeric - first_future_date)), ]
    ...
    rm(ch) # [AW] why is this being closed now as opposed to earlier?
    ...
    campaign_response_old <- campaign_response_final[
      is.na(campaign_response_final$response_date) == "FALSE" & 
      as.character(campaign_response_final$response_date) != "NULL", ]
    ...
    #Set both new_campaigns projected and called to NA
    new_campaigns$projected <- "NA"
    new_campaigns$Called <- "NA"
    
    #Fix new campaigns response datw
    ...
    output <- list("campaign_response_final" = campaign_response_final, 
                   "current_date_numeric" = current_date_numeric, 
                   "current_date" = current_date, 
                   "campaign_response_1" = campaign_response_1, 
                   "campaign_response_2" = campaign_response_2, 
                   "campaign_response_old" = campaign_response_old, 
                   "first_future_date" = first_future_date, 
                   "campaign_future" = campaign_future, 
                   "campaigns_outstanding" = campaigns_outstanding, 
                   "current_campaigns" = current_campaigns, 
                   "future_days" = future_days,
                   "campaigns_by_day" = campaigns_by_day, 
