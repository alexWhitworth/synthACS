function(full_data, training_data, OOS_data, min_days, estimation_limit_called_proj, momentum_limit, past_proj) { 
    ...
    training_data_1 <- training_data[training_data$campaign_group != "Latest INQ w/out Phone" & training_data$campaign_group != "Latest INQ w/ Phone", ]
    training_data_2 <- training_data[training_data$campaign_group == "Latest INQ w/out Phone" | training_data$campaign_group == "Latest INQ w/ Phone", ]
    
    #Step 1, get sum of responsders on training_data sample, by days_to_response, rounds, round, class_of_mail. This is agg_stats_1
    agg_stats_1 <- aggregate(as.numeric(training_data_1$responders), by = list(training_data_1$days_to_response, training_data_1$rounds, training_data_1$class), FUN=sum, na.rm = "TRUE")
    
    #Step 2, get sum of leads on training_data sample, by days_to_response, rounds, round, class_of_mail. This is agg_stats_1a
    agg_stats_1a <- aggregate(as.numeric(training_data_1$leads), by = list(training_data_1$days_to_response, training_data_1$rounds, training_data_1$class), FUN=sum, na.rm = "TRUE")
    ...
    agg_stats_1_a <- aggregate(as.numeric(training_data_2$responders), by = list(training_data_2$days_to_response), FUN=sum, na.rm = "TRUE")
    
    #Step 2, get sum of leads on training_data sample, by days_to_response, rounds, round, class_of_mail. This is agg_stats_1a
    agg_stats_1a_a <- aggregate(as.numeric(training_data_2$leads), by = list(training_data_2$days_to_response), FUN=sum, na.rm = "TRUE")
    ...
    sum_responders <- aggregate(as.numeric(training_data_norm$responders), by = list(training_data_norm$response_date), FUN=sum, na.rm = "TRUE")
    ...
    norm_responders <- aggregate(as.numeric(as.character(training_data_unique_norm$sum_responders)), by = list(training_data_unique_norm$response_day_of_week), FUN=sum, na.rm = "TRUE")
    norm_called <- aggregate(as.numeric(as.character(training_data_unique_norm$Called)), by = list(training_data_unique_norm$response_day_of_week), FUN=sum, na.rm = "TRUE")
    ...
    norm_RR_leads <- aggregate(as.numeric(as.character(training_data_unique_norm$leads)), by=list(training_data_unique_norm$response_day_of_week), FUN=sum, na.rm="TRUE")
    ...
    for(i in 1:nrow(OOS_data_9)) {
      OOS_data_9$projected_response_rate[i] <- sum(OOS_data_9$linear_interp_single_1st[i],  OOS_data_9$linear_interp_single_3rd[i], OOS_data_9$linear_interp_multiple_1st[i],  OOS_data_9$linear_interp_multiple_3rd[i],
      na.rm="TRUE")
    }
    
    #Replace latest inq campaings with latest inq estimate
    OOS_data_9$projected_response_rate <- ifelse(as.character(OOS_data_9$campaign_group) == "Latest INQ w/ Phone" | as.character(OOS_data_9$campaign_group) == "Latest INQ w/out Phone",
    ...
    if(past_proj == "FALSE") {
        ...
        total_projected_responses_1 <- merge(total_projected_responses, OOS_unique, by.x="Group.1", by.y="OOS_unique.response_date", all.x="TRUE")
    ...
    if(past_proj == "TRUE") {
        ...
        Tuesday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Tuesday")
        Monday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Monday")
        Wednesday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Wednesday")
        Thursday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Thursday")
        Friday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Friday")
        Saturday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Saturday")
        Sunday_reg <- lm(training_data_unique_proj$Called ~ 0 + training_data_unique_proj$projected, weights = training_data_unique_proj$Called, subset = training_data_unique_proj$response_day_of_week == "Sunday")
        ...
        OOS_data_9$projected_responses <- OOS_data_9$projected_response_rate * as.numeric(as.character(OOS_data_9$leads))  
        OOS_data_9$projected_responses_new <- OOS_data_9$projected_response_rate * as.numeric(as.character(OOS_data_9$leads)) * as.numeric(as.character(OOS_data_9$day_reg_adj))
        ...
        total_projected_responses_new <- aggregate(OOS_data_9$projected_responses_new, by=list(OOS_data_9$response_date), FUN=sum, na.rm = "TRUE")
        ...
        total_projected_responses_1 <- merge(total_projected_responses_1, total_projected_responses_new, by="Group.1", all.x="TRUE")
        total_projected_responses_1$proj_new <- total_projected_responses_1$x
        
        total_projected_responses_1 <- merge(total_projected_responses_1, total_projected_responses_new, by="Group.1", all.x="TRUE")
        
        #Form unique OOS data
        OOS_data_9_unique <- data.frame(OOS_data_9$response_date, OOS_data_9$year_response_date, OOS_data_9$month_response_date, OOS_data_9$day_response_date, OOS_data_9$response_day_of_week)
        OOS_data_9_unique <- OOS_data_9_unique[!duplicated(OOS_data_9_unique$OOS_data_9.response_date), ]
        
        total_projected_responses_1 <- merge(total_projected_responses_1, OOS_data_9_unique, by.x="Group.1", by.y="OOS_data_9.response_date", all.x="TRUE")
        total_projected_responses_1 <- total_projected_responses_1[order(total_projected_responses_1$OOS_data_9.year_response_date, total_projected_responses_1$OOS_data_9.month_response_date,
        ...
        total_projected_responses_1$perc_error <- (total_projected_responses_1$proj_new - total_projected_responses_1$OOS_unique.Called) / total_projected_responses_1$OOS_unique.Called
        ...
        for(i in 1:nrow(total_projected_responses_1)) {
            ...
            if(as.character(total_projected_responses_1$OOS_data_9.response_day_of_week[i]) == "Saturday" | as.character(total_projected_responses_1$OOS_data_9.response_day_of_week[i]) == "Sunday") {
            ...
            total_projected_responses_1$perc_error <- ifelse(is.na(total_projected_responses_1$perc_error), 0, total_projected_responses_1$perc_error)
            ...
            total_projected_responses_1$error_by_called[i] <- total_projected_responses_1$perc_error[i] * total_projected_responses_1$Called_weights[i]
            
            if(i >= 2) {
                total_projected_responses_1$projected_new[i] <- total_projected_responses_1$proj_new[i] / (1 + (sum(total_projected_responses_1$error_by_called[1:(i-1)]) / sum(total_projected_responses_1$Called_weights[1:(i-1)])) )
            }
    ...
    if(past_proj == "TRUE") {
        output <- list("total_projected_responses_1" = total_projected_responses_1, "total_projected_responses_no_weekends" = total_projected_responses_no_weekends, "full_data" = full_data, "RMSE_new" = RMSE_new, 
