...
#Query ALL calls for holiday adjustments --> Define all_calls
all_calls <- call_query_all()$called
  # [AW 7/31] NOTE: need to update call_query_all to return a data.frame instead of a list of length == 1
  # returns a SQL query which is sorted by date
  # rewritten function saved as data_pull.R
...
all_call_regs <- all_call_estimates(all_calls)
  # [AW 7/31] Subsets all_calls by category, does a basic scrub for NA, and then aggregates each. 
  # merges separate datasets back together and does some additional munging. Returns list of each
  # categorical subset as well as re-merged data
  # [AW 7/31] replaced this function with like 8 lines of dplyr
  # [AW 7/31] this doesn't do any regressions. It also isn't ever used again. Why have it?

#Create monthly adjustment
monthly_adjust <- monthly_adjustment(all_calls, control_year_min= 2013, 
                                     control_year_max= lubridate::year(Sys.Date()) - 1)
  # [AW 7/31] Inputs: call data, min / max years. 
  # Does some subsetting, then aggregates calls by month. Normalizes monthly calls
  # Returns: list of aggregated monthly call data, averages, and normalized calls.

#strFull weekly call projection
w <- weekly_call_projection_full() # this allocates a LOT of memory
  # [AW 7/31] internal calls to call_query(), DM_call_model_data_load_and_clean(), and monthly_run()
  # returns a list of unmodified returns from these three functions as well as 1-2 additional things
...
Sundays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Sundays$customer_service, 1)) == FALSE, Lag(Sundays$customer_service, 1), 
         Lag(Sundays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Sundays$customer_service, 2)) == FALSE, Lag(Sundays$customer_service, 2), 
         Lag(Sundays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Sundays$customer_service, 3)) == FALSE, Lag(Sundays$customer_service, 3), 
         Lag(Sundays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Sundays$customer_service, 4)) == FALSE, Lag(Sundays$customer_service, 4), 
         Lag(Sundays$customer_service_projection, 4))
...
Mondays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Mondays$customer_service, 1)) == FALSE, Lag(Mondays$customer_service, 1), 
         Lag(Mondays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Mondays$customer_service, 2)) == FALSE, Lag(Mondays$customer_service, 2), 
         Lag(Mondays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Mondays$customer_service, 3)) == FALSE, Lag(Mondays$customer_service, 3), 
         Lag(Mondays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Mondays$customer_service, 4)) == FALSE, Lag(Mondays$customer_service, 4), 
         Lag(Mondays$customer_service_projection, 4))
...
Mondays_Direct_Extension_projection_backup <- 
  ifelse(is.na(Lag(Mondays$Direct_Extension, 1)) == FALSE, Lag(Mondays$Direct_Extension, 1), 
         Lag(Mondays$Direct_Extension_projection, 1)) +
  ifelse(is.na(Lag(Mondays$Direct_Extension, 2)) == FALSE, Lag(Mondays$Direct_Extension, 2), 
         Lag(Mondays$Direct_Extension_projection, 2)) + 
  ifelse(is.na(Lag(Mondays$Direct_Extension, 3)) == FALSE, Lag(Mondays$Direct_Extension, 3), 
         Lag(Mondays$Direct_Extension_projection, 3)) +
  ifelse(is.na(Lag(Mondays$Direct_Extension, 4)) == FALSE, Lag(Mondays$Direct_Extension, 4), 
         Lag(Mondays$Direct_Extension_projection, 4))
...
Tuesdays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Tuesdays$customer_service, 1)) == FALSE, Lag(Tuesdays$customer_service, 1), 
         Lag(Tuesdays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Tuesdays$customer_service, 2)) == FALSE, Lag(Tuesdays$customer_service, 2), 
         Lag(Tuesdays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Tuesdays$customer_service, 3)) == FALSE, Lag(Tuesdays$customer_service, 3), 
         Lag(Tuesdays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Tuesdays$customer_service, 4)) == FALSE, Lag(Tuesdays$customer_service, 4), 
         Lag(Tuesdays$customer_service_projection, 4))
...
Tuesdays_Direct_Extension_projection_backup <-
  ifelse(is.na(Lag(Tuesdays$Direct_Extension, 1)) == FALSE, Lag(Tuesdays$Direct_Extension, 1), 
         Lag(Tuesdays$Direct_Extension_projection, 1)) +
  ifelse(is.na(Lag(Tuesdays$Direct_Extension, 2)) == FALSE, Lag(Tuesdays$Direct_Extension, 2), 
         Lag(Tuesdays$Direct_Extension_projection, 2)) + 
  ifelse(is.na(Lag(Tuesdays$Direct_Extension, 3)) == FALSE, Lag(Tuesdays$Direct_Extension, 3), 
         Lag(Tuesdays$Direct_Extension_projection, 3)) +
  ifelse(is.na(Lag(Tuesdays$Direct_Extension, 4)) == FALSE, Lag(Tuesdays$Direct_Extension, 4), 
         Lag(Tuesdays$Direct_Extension_projection, 4))
...
Wednesdays_mktg_direct_projection_backup <- 
  ifelse(is.na(Lag(Wednesdays$mktg_direct, 1)) == FALSE, Lag(Wednesdays$mktg_direct, 1), 
         Lag(Wednesdays$mktg_direct_projection, 1)) +
  ifelse(is.na(Lag(Wednesdays$mktg_direct, 2)) == FALSE, Lag(Wednesdays$mktg_direct, 2), 
         Lag(Wednesdays$mktg_direct_projection, 2)) + 
  ifelse(is.na(Lag(Wednesdays$mktg_direct, 3)) == FALSE, Lag(Wednesdays$mktg_direct, 3), 
         Lag(Wednesdays$mktg_direct_projection, 3)) +
  ifelse(is.na(Lag(Wednesdays$mktg_direct, 4)) == FALSE, Lag(Wednesdays$mktg_direct, 4), 
         Lag(Wednesdays$mktg_direct_projection, 4))
Wednesdays$mktg_direct_projection <- Wednesdays_mktg_direct_projection_backup 

Wednesdays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Wednesdays$customer_service, 1)) == FALSE, Lag(Wednesdays$customer_service, 1), 
         Lag(Wednesdays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Wednesdays$customer_service, 2)) == FALSE, Lag(Wednesdays$customer_service, 2), 
         Lag(Wednesdays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Wednesdays$customer_service, 3)) == FALSE, Lag(Wednesdays$customer_service, 3), 
         Lag(Wednesdays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Wednesdays$customer_service, 4)) == FALSE, Lag(Wednesdays$customer_service, 4), 
         Lag(Wednesdays$customer_service_projection, 4))
...
Thursdays_mktg_direct_projection_backup <- 
  ifelse(is.na(Lag(Thursdays$mktg_direct, 1)) == FALSE, Lag(Thursdays$mktg_direct, 1), 
         Lag(Thursdays$mktg_direct_projection, 1)) +
  ifelse(is.na(Lag(Thursdays$mktg_direct, 2)) == FALSE, Lag(Thursdays$mktg_direct, 2), 
         Lag(Thursdays$mktg_direct_projection, 2)) + 
  ifelse(is.na(Lag(Thursdays$mktg_direct, 3)) == FALSE, Lag(Thursdays$mktg_direct, 3), 
         Lag(Thursdays$mktg_direct_projection, 3)) +
  ifelse(is.na(Lag(Thursdays$mktg_direct, 4)) == FALSE, Lag(Thursdays$mktg_direct, 4), 
         Lag(Thursdays$mktg_direct_projection, 4))
Thursdays$mktg_direct_projection <- Thursdays_mktg_direct_projection_backup 

Thursdays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Thursdays$customer_service, 1)) == FALSE, Lag(Thursdays$customer_service, 1), 
         Lag(Thursdays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Thursdays$customer_service, 2)) == FALSE, Lag(Thursdays$customer_service, 2), 
         Lag(Thursdays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Thursdays$customer_service, 3)) == FALSE, Lag(Thursdays$customer_service, 3), 
         Lag(Thursdays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Thursdays$customer_service, 4)) == FALSE, Lag(Thursdays$customer_service, 4), 
         Lag(Thursdays$customer_service_projection, 4))
...
Fridays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Fridays$customer_service, 1)) == FALSE, Lag(Fridays$customer_service, 1), 
         Lag(Fridays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Fridays$customer_service, 2)) == FALSE, Lag(Fridays$customer_service, 2), 
         Lag(Fridays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Fridays$customer_service, 3)) == FALSE, Lag(Fridays$customer_service, 3), 
         Lag(Fridays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Fridays$customer_service, 4)) == FALSE, Lag(Fridays$customer_service, 4), 
         Lag(Fridays$customer_service_projection, 4))
...
Fridays_Direct_Extension_projection_backup <- 
  ifelse(is.na(Lag(Fridays$Direct_Extension, 1)) == FALSE, Lag(Fridays$Direct_Extension, 1), 
         Lag(Fridays$Direct_Extension_projection, 1)) +
  ifelse(is.na(Lag(Fridays$Direct_Extension, 2)) == FALSE, Lag(Fridays$Direct_Extension, 2), 
         Lag(Fridays$Direct_Extension_projection, 2)) + 
  ifelse(is.na(Lag(Fridays$Direct_Extension, 3)) == FALSE, Lag(Fridays$Direct_Extension, 3), 
         Lag(Fridays$Direct_Extension_projection, 3)) +
  ifelse(is.na(Lag(Fridays$Direct_Extension, 4)) == FALSE, Lag(Fridays$Direct_Extension, 4), 
         Lag(Fridays$Direct_Extension_projection, 4))
...
Saturdays_mktg_direct_projection_backup <- 
  ifelse(is.na(Lag(Saturdays$mktg_direct, 1)) == FALSE, Lag(Saturdays$mktg_direct, 1), 
         Lag(Saturdays$mktg_direct_projection, 1)) +
  ifelse(is.na(Lag(Saturdays$mktg_direct, 2)) == FALSE, Lag(Saturdays$mktg_direct, 2), 
         Lag(Saturdays$mktg_direct_projection, 2)) + 
  ifelse(is.na(Lag(Saturdays$mktg_direct, 3)) == FALSE, Lag(Saturdays$mktg_direct, 3), 
         Lag(Saturdays$mktg_direct_projection, 3)) +
  ifelse(is.na(Lag(Saturdays$mktg_direct, 4)) == FALSE, Lag(Saturdays$mktg_direct, 4), 
         Lag(Saturdays$mktg_direct_projection, 4))
Saturdays$mktg_direct_projection <- Saturdays_mktg_direct_projection_backup 

Saturdays_customer_service_projection_backup <- 
  ifelse(is.na(Lag(Saturdays$customer_service, 1)) == FALSE, Lag(Saturdays$customer_service, 1), 
         Lag(Saturdays$customer_service_projection, 1)) +
  ifelse(is.na(Lag(Saturdays$customer_service, 2)) == FALSE, Lag(Saturdays$customer_service, 2), 
         Lag(Saturdays$customer_service_projection, 2)) + 
  ifelse(is.na(Lag(Saturdays$customer_service, 3)) == FALSE, Lag(Saturdays$customer_service, 3), 
         Lag(Saturdays$customer_service_projection, 3)) +
  ifelse(is.na(Lag(Saturdays$customer_service, 4)) == FALSE, Lag(Saturdays$customer_service, 4), 
         Lag(Saturdays$customer_service_projection, 4))
